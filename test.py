...
# Testing regression model without retraining0
...
def normalizeInputs( train, test ):
    ...
    for i in range(numFeatures):
        lenX, meanX, stdX = taftdata.normalize( train[:,i] )
        if lenX is None:
            return None
        lenX, _, _ = taftdata.normalize( test[:,i], meanX=meanX, stdX=stdX )
        if lenX is None:
            return None
    return "Ok"
# end of def 
